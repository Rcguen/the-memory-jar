...
export function PhotoKeepsake(props: KeepsakeLayoutProps) {
  ...
  const { data: url, isError, isLoading } = useQuery({ queryKey: ["signedAttachmentUrl", primary?.id, primary?.url], queryFn: () => memoryService.getAttachmentUrlAsync(primary!.file_type, primary!.url), enabled: Boolean(primary) && !isLocked, staleTime: 1000 * 60 * 30 });
  const preview = url ? <img src={url} alt="" className="h-full w-full object-cover object-center" loading="lazy" /> : <div className="flex h-full items-center justify-center text-stone-500 bg-black/5"><ImageIcon className={isLoading ? "h-5 w-5 animate-pulse" : "h-5 w-5"} /><span className="sr-only">{isError ? "Photo unavailable" : "Loading photo"}</span></div>;
